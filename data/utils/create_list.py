...
def ReadFileBypath(path):
    ...
    for dirpath, dirnames, filenames in os.walk(path):
        for filename in filenames:
            ...
            img = img.replace('/mnt/bee9bc2f-b897-4648-b8c4-909715332cb4/wy/data/', '')
            label = img.replace('img.tif', 'label.png').replace('/image/', '/label/')
            file.append(img + '\t' + label)
    df = pd.DataFrame(file, columns=['one'])
    # df = df.sample(frac=0.05, random_state=1)
    df.to_csv("/mnt/bee9bc2f-b897-4648-b8c4-909715332cb4/wy/NAS/rnas/data/lists/normal_FBP/fbp_756_train.lst", columns=['one'], index=False, header=False)
    
def ReadFileBypath_fbp(path):
    imgs = glob.glob(('{}*.tif'.format(path)))
    file = []
    for img in imgs:
        img = img.replace('/mnt/bee9bc2f-b897-4648-b8c4-909715332cb4/wy/data/', '')
        label = img.replace('Image__8bit_NirRGB', 'Annotation__index').replace('.tif', '_24label.png')
        file.append(img + '\t' + label)
    df = pd.DataFrame(file, columns=['one'])
    # df = df.sample(frac=0.05, random_state=1)
    df.to_csv("/mnt/bee9bc2f-b897-4648-b8c4-909715332cb4/wy/NAS/DNAS/data/lists/FBP/fbp_ori_test.lst", columns=['one'], index=False, header=False)
...
if __name__ == '__main__':
    ...
    # mergeFile()
    # changeFile()
    # path = "/mnt/bee9bc2f-b897-4648-b8c4-909715332cb4/wy/data/ori/Image__8bit_NirRGB/test/"
    # ReadFileBypath_fbp(path)
    # sampleFile()
    # copyFile()
    path = "/mnt/bee9bc2f-b897-4648-b8c4-909715332cb4/wy/data/FBP/756/image/train/"
    ReadFileBypath(path)